...
// Public root-level health check (used directly by k8s probes, not via CloudFront)
...
// All application API routes live under /api/v1 so they route correctly
// through the CloudFront "/api/*" cache behavior to the ALB-Backend origin.
apiV1 := router.Group("/api/v1")
{
// Also expose health under /api/v1/health per spec, in addition to root /health
